...
#make a trial duration column to test_table by start and end time 
...
# Add sample rate number of tags during test and test_duration to test_table
test_table <- test_table %>%
      mutate(sample_rate = 2) %>%
      mutate(N_tags =5)
...
Tag_Data <- test_table %>%
      select(Dyad_test_type, distance_cm, sample_rate, N_tags, trial_duration) %>%
...
```{r}
#Show just the tags read by one tag for known period of time

#checking sample rate for proxlog 
Tag_Data %>% 
     filter(Date_Time > as.POSIXct('2023-11-10 16:44:00', tz= "America/New_York")) %>%
      filter(Date_Time < as.POSIXct('2023-11-10 16:45:00', tz= "America/New_York")) %>%
     filter(RX_TX == "pW2_pN1") %>%
      #filter(RX_Node == "pW2") %>%
      ggplot(aes(x=Date_Time, y=tag)) +
       geom_point()
#looks like was transmitting every 2 seconds 

#checking sample rate for dulog
Tag_Data %>% 
     filter(Date_Time > as.POSIXct('2023-11-10 12:30:00', tz= "America/New_York")) %>%
      filter(Date_Time < as.POSIXct('2023-11-10 12:31:00', tz= "America/New_York")) %>%
     filter(RX_TX == "dE73_dN69") %>%
      #filter(RX_Node == "pW2") %>%
      ggplot(aes(x=Date_Time, y=tag)) +
       geom_point() #+

#both 2 seconds sample rate, but proxlog is patchier
...
#ggsave("RSSI_distance_comparison.jpg")
...
# ggsave("RSSI_variance_comparison.jpg")
...
#accuracy by dyad
 accuracy_by_dyad <- 
      comparison %>% 
      group_by(test_type_tag, Dyad) %>% #Dyad
...
sum_accuracy_dyad <-
      accuracy_by_dyad %>%
      group_by(test_type_tag) %>%
      summarize(mean_pct_detected = mean(pct_detected), sd= sd(pct_detected), mean_pct_missing = mean(pct_missing))
sum_accuracy_dyad

   dyad_accuracy_plot <-
         ggplot(data = accuracy_by_dyad, aes( x = test_type_tag, y = pct_detected, color = Dyad )) +
      geom_point() +
      geom_point(data = sum_accuracy_dyad, aes(x=test_type_tag, y=mean_pct_detected), shape = 3, color = "red" )+
      ylim(0,100) +
      theme(axis.text.x = element_text(angle = 45)) +
...
accuracy_by_tag <- 
      comparison %>% 
      group_by(test_type_tag, TX_Node) %>% #Dyad or TX_Node
      summarize(n=n(), trial_duration = max(trial_duration), sample_rate = max(sample_rate), N_tags= max(N_tags) ) %>%
...
sum_accuracy_tag
      ...
      tag_accuracy_plot <-
            ggplot(data = accuracy_by_tag, aes( x = test_type_tag, y = pct_detected, color = TX_Node )) +
      geom_point() +
      geom_point(data = sum_accuracy_tag, aes(x=test_type_tag, y=mean_pct_detected), shape = 3, color = "red" )+
       ylim(0,100) +
       theme(axis.text.x = element_text(angle = 45)) +
...
```{r dulog accuracy rates}
#comparing all  accuracy rates for dulog
dulog_summary <- 
      Tag_Data %>%
      filter(tag=="dulog") %>%
accuracy_test(., Title = "All trials")
dulog_summary

dulog_summary[[3]]
#ggsave("meetings_detected_dyad_dulog.jpg")
dulog_summary[[5]]
#ggsave("meetings_detected_tag_dulog.jpg")
dulog_summary[[2]]
#write.csv(dulog_summary[[2]], "dulog_accuracy_by_test.csv")
```
time function 
```{r}
...
class(Tag_Data_Jan26$Date_Time)


```


performance of dyads over the course of tests
```{r}

Tag_Data %>%
      filter(tag=="dulog") %>%
time_fn(data= ., Trial = test_type, Trial_type = "fifty_parallel", breaks = "min", group= Dyad, expected = 60)
#ggsave("dulog_differences in meetings between dyads over time.jpg")

Tag_Data %>%
      filter(tag=="dulog") %>%
time_fn(data= ., Trial = test_type, Trial_type = "fifty_parallel", breaks = "min", group= TX_Node, expected = 60)
#ggsave("dulog_differences in meetings between tags over time.jpg")


Tag_Data %>%
      filter(tag=="proxlog") %>%
time_fn(data= ., Trial = test_type, Trial_type = "fifty_parallel", breaks = "min", group= Dyad, expected = 60)
#ggsave("proxlog_differences in meetings between dyads over time.jpg")

Tag_Data %>%
      filter(tag=="proxlog") %>%
time_fn(data= ., Trial = test_type, Trial_type = "fifty_parallel", breaks = "min", group= TX_Node, expected = 60)
#ggsave("proxlog_differences in meetings between tags over time.jpg")
```



# comparing all accuracy rates for proxlogs

```{r proxlog accuracy rates}
#comparing all  accuracy rates for proxlog, given that this test was done with very high download rates
proxlog_summary <- 
      Tag_Data %>%
      filter(tag=="proxlog") %>%
accuracy_test(., Title = "All trials")
...
proxlog_summary[[3]]
#ggsave("meetings_detected_dyad_proxlog.jpg")
proxlog_summary[[5]]
#ggsave("meetings_detected_tag_proxlog.jpg")
proxlog_summary[[2]]
#write.csv(proxlog_summary[[2]], "proxlog_accuracy_by_test.csv")
...
glm(pct_detected ~ Dyad + test_type_tag, data = dulog_summary[[1]])
glm(pct_detected ~  test_type_tag, data = dulog_summary[[1]])
...
```{r}
#write.csv(Tag_Data, "Nov5_logger_testing.csv")
# write.csv(obs, "S-differences between species.csv")
Tag_Data_Nov10 <-
      Tag_Data %>%
      select(!Dyad_test_type) %>% #drop dyad dummy variable 
      relocate(Dyad, Date_Time, RSSI, test_type)
